...
def print_tasks(tasks: list[dict], title: str = "Open Tasks"):
    ...
    table.add_column("CD", justify="right", style="dim")

    for t in tasks:
        tid = t["id"][:8]
        best = f"{t['best_score']:.2f}" if t.get("best_score") is not None else "-"
        pool = f"{t['pool_balance']} $AXN"
        cd = f"{t.get('cooldown_seconds', 1)}s"
        table.add_row(tid, t["title"][:50], t.get("eval_type", ""), best, pool, cd)
...
def build_mining_panel(
    ...
    lines = [
        ...
        f"Tokens: {total_tokens:,}  Cost: [yellow]{cost_str}[/]",
    ]
